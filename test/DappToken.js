...
contract('DappToken', function(accounts) {
	...
	it('innitializes the contract with correct values', function() {
	...
	it('transfers token ownership', function() {
		...
		return tokenInstance.transfer.call( accounts[1], 9999999999999999999999 );
			...
			assert.equal( balance.toNumber(), 750000, 'deducts the amount from descending account')
		});	
	});
	...
	it('approves tokens for delegated transfer', function() {
		return DappToken.deployed().then( function(instance)  {
			...
			assert.equal(success, true, 'returned successfully')
	...
	it('handles delegated token transfers', function(){
		return DappToken.deployed().then( function(instance)  {
			...
			return tokenInstance.transferFrom(fromAccount, toAccount, 9999, { from: fromAccount } );
		}).then( assert.fail ).catch( function(error) {
